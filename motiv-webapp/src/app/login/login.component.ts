...
export class LoginComponent implements OnInit {
  ...
  loginWithCredentials(): void {
    ...
    this.authService.login(this.email, this.password, (err, currUser) => {
      if (err){
  ...
  signInWithGoogle(): void {
    this.errorMessage = undefined;
    this.authService.loginWithGoogle((err, currUSer) => {
      if (err){
        this.errorMessage = err;
      } else {
        this.onLoginResult();
  ...
  onLoginResult() {
    ...
    this.zone.run(() => { this.router.navigateByUrl('dashboard').then((status) => {
